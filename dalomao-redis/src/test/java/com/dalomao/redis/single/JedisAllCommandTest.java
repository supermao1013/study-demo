...
public class JedisAllCommandTest {
	...
	public void testCommand() {
		...
		for (int i = 0; i < 100; i++) {
			// 设值
			JedisUtils.set("n" + i, String.valueOf(i));
		}
		System.out.println("keys from redis return =======" + JedisUtils.keys("*"));
	}
...
//		jedis.auth("12345678");
		...
		pipe.multi();
		pipe.set("name", "dalomao"); // 调值
		pipe.incr("age");// 自增
		pipe.get("name");
		pipe.discard();
		// 将不同类型的操作命令合并提交，并将操作结果以list返回
...
//		jedis.auth("12345678");
	...
	@Test
	public void ttt() {
		Jedis jedis = new Jedis("127.0.0.1", 6379, 2000, 2000, false, null, null, null);
		jedis.connect();
		jedis.auth("12345678");
